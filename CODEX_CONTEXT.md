...
> 当前产品版本：2026-09-08 中文手机原型。第 25 节为最新拍摄交互约束，第 24 节为创建入口约束；先浏览模板，进入详情点击「应用」后才引导创建分身的规则继续保留。
...
7. 创建成功后「使用这个分身」设为当前身份，自动脸型分析约 1 秒；保留既有脸型结果、推荐和来源回流逻辑。以上均为模拟生成，不执行真实照片质量或脸型识别。
