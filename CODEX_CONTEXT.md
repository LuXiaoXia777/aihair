...
> 当前产品版本：2026-09-08 中文手机原型。第 24 节为最新创建入口约束；先浏览模板，进入详情点击「应用」后才引导创建分身的规则继续保留。
...
3. 三张拍齐后展示三张缩略图，点击唯一「确定」按钮开始生成分身。
...
- `src/screens/PhotoUpload.tsx`：真实本地相册多选与三角度拍摄完成确认。
...
- 相机 → 正脸/左侧脸/右侧脸连续拍摄 → 三张照片完成页 →「确定」开始生成。第 23 节的无逐张确认规则保留；本次新增的是三张完成后统一确认。
